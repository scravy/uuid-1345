...
var clockSeq = (randomHost[3] << 8 | randomHost[4]) & 0x3fff;
...
function uuidgen(nodeId, callback) {
    ...
    if (delta < 0) {
        ...
        process.nextTick(uuidgen.bind(null, nodeId, callback));
        return;
    }
...
module.exports = function v1(options, callback) {
    ...
    if (nodeId === undefined) {
        if (macAddress === null) {
            getMAC(function (err, addr) {
                macAddress = err ? randomHost : parseMAC(addr);
                uuidgen(macAddress, callback);
            });
        } else {
            uuidgen(macAddress, callback);
        }
    ...
    if (nodeId instanceof Buffer) {
        ...
        uuidgen(nodeId, callback);
        return;
    }
    if (macRegex.test(nodeId)) {
        uuidgen(parseMAC(nodeId.toLowerCase()), callback);
        return;
    }
    callback(invalidMacAddress, null);
};
